...
import org.goat.util.CommandParser;
import org.goat.util.DICTClient;
import org.goat.util.Definition;
import org.goat.util.UrbanDictionary;
...
public class Define extends Module {
    ...
    private void define(Message m) {
        ...
        if(null == word || word.equals("") ) {
            ...
            if (wordgameMod != null && (wordgameMod.inAllChannels || wordgameMod.getChannels().contains(m.getChatname()))) {
                String lastWord = WordGame.getLastWinningWord(m.getChatname()) ;
                if(lastWord != null)
                    word = lastWord ;
                else {
        ...
        if (definitionList.isEmpty()) {
            ...
            if (0 == matchList.length) {
                m.reply(reply + "  Couldn't find any alternate spelling suggestions.") ;
            } else {
                String suggestions = "" ;
                for (String[] aMatchList : matchList) suggestions += " " + aMatchList[1];
                suggestions = suggestions.replaceAll("\"", "") ;
                suggestions = suggestions.trim() ;
                m.reply(reply + "  Suggestions: " + suggestions) ;
            }
        }
    ...
    private void dictionaries(Message m) {
        m.reply(dictionaries()) ;
    }

    private void dictionary(Message m) {
        DICTClient dc = getDICTClient(m) ;
        String code = m.getModText().trim() ;
        String line = "" ;
        String[][] dbList = dc.getDatabases() ;
        dc.close() ;
        boolean found = false ;
