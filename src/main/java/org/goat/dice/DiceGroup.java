package org.goat.dice;

import java.util.ArrayList;
import static org.goat.dice.Dice.*;
...
public class DiceGroup {
    ...
    public String toString() {
        StringBuilder term = new StringBuilder( BOLD + dice.size() + "d" + size +  END_BOLD + ":" + UNDERLINE );
        for( int i=0; i<dice.size(); i++ ) {
            term.append(dice.get(i).getResult());
            if(i<(dice.size()-1))
                term.append(",");
        }
        term.append(END_UNDERLINE + ":" + getTotal());
