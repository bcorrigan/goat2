...
import org.goat.util.CountdownSolver;
...
public class CountDown extends Module implements Runnable {
    ...
    public void processChannelMessage(Message m) {
        if(gameOn) {
            ...
            if( checkAttemptValid( attempt ) ) {
                ...
                try {
                    ...
                    if(bestAnswer == null || bestAnswer.getDistance( targetNumber )>possibleAnswer.getDistance( targetNumber ) ) {
                        ...
                        m.reply( Constants.BOLD + m.getSender() + Constants.END_BOLD
            ...
            m.reply(Constants.UNDERLINE + Constants.BOLD + "***" + Constants.END_BOLD + Constants.END_UNDERLINE
                    + " New Numbers: " + Constants.BOLD
                    + formatNumbers( sourceNumbers )
                    + Constants.END_BOLD + " Target: " + Constants.BOLD + targetNumber + Constants.END_BOLD);
        }
    }
    ...
    private void finaliseGame() {
        ...
        if( bestAnswer.getAnswer() == bestPossibleAnswer ) {
            String reply = Constants.BOLD + bestAnswer.getUsername() + Constants.END_BOLD
    ...
    private boolean numbersCorrect( int[] num ) {
        ...
        for (int aNum : num) {
            boolean removed = sourceNumList.remove(Integer.valueOf(aNum));
            if (!removed)
                return false;
        }
        return true;
